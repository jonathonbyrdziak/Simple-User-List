...
class ResTable extends JTable
{
	...
	{
		//reasons to fail
		if (!isset($this->description)) return false;
		if ($length === false) return $this->description;
		
		return substr(strip_tags($this->description), 0, $length);
	}
	 ...
	 * @return array
	 */
	function getList( $filter = null, $limit = null ) 
	{
		//initializing variables
		...
		if (is_null($limit)) $limit = JRequest::getVar('limit',20);
	...
	function query( $query )
	{
		switch (strtolower( substr(trim($query),0,6) ))
		{
			...
			default: case 'insert': case'update':
				$this->_db->setQuery( $query );
	...
	{
		//initializing variables
		$query = "SHOW TABLES LIKE '`$this->_tbl`'";
